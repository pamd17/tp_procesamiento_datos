...
/*
    Tuve problemas para compilar y ejecutar correctamente, entonces use esto:
        gcc -D__USE_MINGW_ANSI_STDIO -o Tarea2 Tarea2.c
        .\Tarea2.exe

    Debido a que tenia problemas con Windows 
    El flag -D__USE_MINGW_ANSI_STDIO es necesario en Windows con MinGW
    para que sscanf pueda leer valores "nan" en el CSV.
*/
#include <ctype.h>
#include <stdint.h>
#include <stdio.h>
...
double calcular_longitud_temporal(int total, double fs) {
    ...
    // 2. Calcular el tiempo total como: tiempo = total / fs
...
int buscar_indice_primera_sync(const Datos registros[], int total) {
    ...
    for (int i = 0; i < total; i++) {
        ...
        if (registros[i].sync != 0) {
            return i; // 3. Devolver ese indice
        }
    }
    return -1; // 4. Si no hay ninguna, devolver -1
}
...
int contar_transiciones_S3_S0(const Datos registros[], int inicio, int fin) {
    ...
    return pasos; // 4. Devolver la cantidad de pasos detectados
}
...
double calcular_velocidad_marcha(int muestras_sync, double fs) {
    ...
    double tiempo = (double)muestras_sync / fs;
    
    // 2. Usar la distancia util de 6 metros 
    // 3. Devolver distancia / tiempo (v = 6 / T_sync) 
...
double calcular_velocidad_pasos(int pasos, int muestras_pasos, double fs) {
    ...
    // 2. Calcular el tiempo de la ventana [cite: 80]
...
double calcular_longitud_zancada(double velocidad_marcha, double velocidad_pasos) {
    ...
    // 2. Calcular: longitud = velocidad_marcha / velocidad_pasos
